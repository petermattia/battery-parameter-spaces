...
# calculate bias-correctedearly prediction error
...
fig5 = ax0.plot(np.transpose(predicted_lifetimes), np.transpose(final_lifetimes),'o')
...
## Lifetimes plot
plt.figure()
fig6 = plt.errorbar(oed_means,final_means,fmt='o',yerr=final_stdevs)
ax = plt.gca()
plt.xlim([0,1300])
plt.ylim([0,1300])
ax.set_aspect('equal', 'box')
...
## Rankings plot
plt.figure()
fig7 = plt.plot(oed_ranks,final_ranks,'o')
ax = plt.gca()
plt.xlim([0,10])
plt.ylim([0,10])
ax.set_aspect('equal', 'box')
