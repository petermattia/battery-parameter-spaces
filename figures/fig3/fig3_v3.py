...
for k, batch_idx in enumerate(batches_to_plot):
    with sns.axes_style('white'):
        if k==0:
            ax = fig.add_axes([0.05,0.45,0.29,0.29],projection='3d')
            ax.set_title('b', loc='left', weight='bold')
        elif k==4:
            ax = fig.add_axes([0.67,0.45,0.29,0.29],projection='3d')
        else:
            ax = fig.add_axes([0.10+0.15*k,0.48,0.2,0.2],projection='3d')
...
#plt.tight_layout()
plt.savefig('fig3_v3.png',bbox_inches='tight')
plt.savefig('fig3_v3.pdf',bbox_inches='tight',format='pdf')