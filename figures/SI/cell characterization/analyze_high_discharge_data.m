...
%% Load xlsx file
if ~exist('data','var')
    data = xlsread('2018-05-30_highdischarge_12_Channel_12.xlsx', 'Channel_12_1');
end

%% Extract columns of interest
t = data(:,2)./60; % test time, seconds -> minutes
step_idx = data(:, 4); % step index
V = data(:,6); % voltage
I = data(:,7); % current
Qc = data(:,8); % charge capacity
...
for k = 1:n_cycles
    ...
    Qd_cycle{k} = Qd(step_indices) - Qd(step_indices(1));
...
%% Save data
save('ch12.mat','V_cycle', 'I_cycle', 'Qd_cycle', 'T_cycle', 'I_leg')
...
%% Plot V vs Qd
for k = 1:n_cycles
    plot(Qd_cycle{k}, V_cycle{k}, 'color', colors_p(k,:)), hold on
end
legend(I_leg), legend boxoff
...
figure
for k = 1:n_cycles
    plot(Qd_cycle{k}, T_cycle{k}, 'color', colors_b(k,:)), hold on
end
legend(I_leg, 'location','northwest'), legend boxoff
