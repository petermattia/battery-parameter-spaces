...
class BayesGap(object):
	...
	def run(self):
		...
		if round_idx == 0:
			...
			print('Parameters and observed (non-standardized) lifetime for the selected arm are', resampled_lifetimes)
		...
		est_mean = (nonstd_upper_bounds+nonstd_lower_bounds)/2
		best_arm_idx = np.argmax(est_mean)
		rank_idx = np.argsort(-est_mean) # sort in descending order
		
		best_arm_params = param_space[best_arm_idx]
		print(nonstd_upper_bounds, np.sort(nonstd_upper_bounds), est_mean, np.sort(est_mean))
		
		print('True lifetimes:', '\t'.join(map(str, self.population_lifetimes)))
		print('True ranks:', '\t'.join(map(str, np.argsort(-self.population_lifetimes))))
		print('Predicted ranks:', '\t'.join(map(str, rank_idx)))
		
		print('Best arm until round', round_idx, \
		'is', best_arm_params, \
		'with population lifetime', self.population_lifetimes[rank_idx[0]])
		print()

		if round_idx != self.max_budget: # write sampled lifetimes for next round
			with open(next_batch_lifetimes_file, 'w+') as outfile:
				...
				for arm, policy in zip(batch_arms, batch_policies):
					...
					print('Arm selected in this round has idx', arm, 'with policy params', policy)
	...
	def get_parameter_space(self):
		...
		sampled_lifetimes = data[:, 3:3+self.pop_budget]
		self.sampled_lifetimes = sampled_lifetimes.tolist()
		for policy_idx, policy_lifetimes in enumerate(self.sampled_lifetimes):
			self.sampled_lifetimes[policy_idx] = [lifetime for lifetime in policy_lifetimes if not np.isnan(lifetime)]
		
		population_data = np.genfromtxt(os.path.join('./oed_vs_random_validation', 'final_results.csv'), delimiter=',', skip_header=1)
		population_data = population_data[self.train_policy_idx, :] # shuffle according to provided permutation
		self.population_lifetimes = np.nanmean(population_data[:, 3:3+self.pop_budget], axis=-1)
...
def parse_args():
	...
	parser.add_argument('--train_policy_idx', nargs='*', default=[0,1,5,2,3,6,4,7], type=int,
						help='list of train policies')
	parser.add_argument('--pop_budget', default=5, type=int,
	...
	parser.add_argument('--apply_correction', dest='apply_correction', action='store_true', default=False,
                        help='update likelihood_std, standardization_mean/std for cell degradation')
...
def main():
	...
	if args.apply_correction:
		# args.standardization_mean = 759.
		# args.standardization_std = 173
		# if not args.early_pred:
		# 	args.likelihood_std = 99
		# else:
		# 	args.likelihood_std = 140

		args.standardization_mean = 772.3
		args.standardization_std = 170.5
		if not args.early_pred:
			args.likelihood_std = 99.2
		else:
			args.likelihood_std = math.sqrt(99.2**2+95.8**2)
