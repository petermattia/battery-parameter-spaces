...
num_policies = 8
pop_budget = 5
max_budget = num_policies*pop_budget-10
...
for round_idx in range(max_budget+1):
	...
	for seed in range(2000):
		...
		for grid_policy in grid_policies:
			emp_policy_lifetimes[grid_policy].append(np.random.choice(early_sample_lifetimes[grid_policy]))
		avg_emp_policy_lifetimes = []
		for policy_idx in range(num_policies):
			if len(emp_policy_lifetimes[policy_idx])>0:
				avg_emp_policy_lifetimes.append(np.mean(np.array(emp_policy_lifetimes[policy_idx])))
...
plt.figure()
start_idx = 0
end_idx = max_budget+1
baseline_cycles_means[0]=0
grid_cycles_means[0] = 0
baseline_means[0] = grid_means[0]
baseline_stds[0] = grid_stds[0]
c = next(color)
print(len(np.arange(start_idx,end_idx)), len(baseline_means[start_idx:end_idx]))
plt.errorbar(100*np.arange(start_idx,end_idx), \
...
plt.legend()
plt.savefig(os.path.join(logdir, 'plot_early.png'))
plt.show()
