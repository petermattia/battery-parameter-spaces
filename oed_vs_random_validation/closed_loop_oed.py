...
class BayesGap(object):
	...
	def __init__(self, args):
		...
		self.seed = args.seed+1000*args.round_idx
		...
		self.apply_correction = args.apply_correction
		...
		if round_idx != self.max_budget: # write sampled lifetimes for next round
			with open(next_batch_lifetimes_file, 'w+') as outfile:
				...
				for arm, policy in zip(batch_arms, batch_policies):
					resampled_lifetime = np.array([sim(policy[0], policy[1], policy[2], \
							variance=True, seed=self.seed, \
							early_pred=self.early_pred, \
	...
	def get_parameter_space(self):
		...
		population_lifetimes = []
		for policy in policies.tolist():
			population_lifetimes.append(sim(policy[0], policy[1], policy[2], variance=False))

		self.population_lifetimes = np.array(population_lifetimes)
