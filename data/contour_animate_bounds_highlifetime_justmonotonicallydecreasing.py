...
param_space2 = np.zeros((param_space.shape[0],param_space.shape[1]+1))
c4_array = 0.2/(1/6 - (0.2/param_space[:,0] + 0.2/param_space[:,1] + 0.2/param_space[:,2]))
param_space2[:,0:-1] = param_space
param_space2[:,-1] = c4_array
...
def make_frame(k2):
    for k, c3 in enumerate(C3list):
        ...
        idx_subset1 = np.where(policy_subset[:,0]>=policy_subset[:,1])[0]
        idx_subset2 = np.where(policy_subset[:,1]>=policy_subset[:,2])[0]
        idx_subset3 = np.where(policy_subset[:,2]>=policy_subset[:,3])[0]
        idx_subsetA = np.intersect1d(np.intersect1d(idx_subset1,idx_subset2),idx_subset3)
        policy_subset = policy_subset[idx_subsetA]
        lifetime_subset = lifetime_subset[idx_subsetA]
...
anim.save('plots/animation_bounds_highlifetime_justmonotonicallydecreasing.gif', writer='imagemagick', fps=0.5)
